...
class _DetailScreenState extends State<DetailScreen> {
  ...
  Future<List<prefix.Table>> fetchTables() async{
    final response = await http.get('https://us-central1-misa-2021.cloudfunctions.net/rest/Misa/getTables');
    if (response.statusCode == 200) {
      List<prefix.Table> tables = (json.decode(response.body) as List).map((i) =>
          prefix.Table.fromJson(i)).toList();
      return tables;
    } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Bitte auf das Objekt tippen,\ndas Sie auf dem Tisch sehen!'),
